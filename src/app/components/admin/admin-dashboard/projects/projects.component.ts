...
export class ProjectsComponent {
  ...
  onFileSelected(event: any) {
    ...
    if (file) {
      ...
      reader.onload = () => {
        this.selectedProject.base64Encoded = reader.result as string;
      };
    ...
    this.projectService.getProjectById(projectId).subscribe({
      next: (response: any) => {
        ...
        const formattedProjects = response._embedded.map((project: any) => ({
          ...
          base64Encoded: project.base64Encoded?.startsWith("data:image/")
            ? project.base64Encoded
            : `data:image/jpeg;base64,${project.base64Encoded}`
        }));
        this.projectService.projectSubject.next({ projects: formattedProjects });
      },
      error: (err) => {
